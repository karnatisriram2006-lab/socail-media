...
const gradientBlobs = [
  { size: 600, x: "20%", y: "20%", color: "from-warm-500/20", duration: 10, delay: 0 },
  { size: 500, x: "70%", y: "60%", color: "from-warm-400/20", duration: 12, delay: 2 },
  { size: 400, x: "40%", y: "80%", color: "from-warm-300/20", duration: 8, delay: 4 },
];

export default function CommunitySection() {
  return (
    <section className="relative min-h-screen py-32 flex items-center justify-center overflow-hidden bg-black">
      ...
      <div className="relative z-10 text-center">
        ...
        <motion.p
          className="text-xl text-warm-300 mt-6 max-w-2xl mx-auto px-4"
            ...
            <div className="relative w-full max-w-lg h-full">
              {floatingAvatars.map((avatar, i) => (
                <motion.div
                  ...
                  initial={{ x: 0, y: 0 }}
                  animate={{ x: [0, avatar.x, 0], y: [0, avatar.y, 0] }}
                  transition={{ duration: 6, repeat: Infinity, ease: "easeInOut", delay: avatar.delay }}
                >
                  <motion.img
                    src={avatar.src}
                    alt=""
                    className="w-16 h-16 rounded-full border-2 border-white/20 object-cover hover:border-warm-400 transition-colors duration-300"
                    whileHover={{ scale: 1.2, borderColor: "rgba(146, 124, 110, 1)" }}
                  />
                </motion.div>
              ))}
            </div>
