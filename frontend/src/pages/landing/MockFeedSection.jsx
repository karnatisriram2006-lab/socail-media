...
const posts = [
  {
    ...
    caption: 'Golden hour magic captured through the lens. Every moment tells a story waiting to be shared.',
    initialLikes: 1423,
    comments: [
  ...
  },
  {
    ...
    caption: 'Found this hidden gem in the heart of the city. Architecture that takes your breath away.',
    initialLikes: 2891,
    comments: [
  ...
  },
  {
    ...
    caption: 'Nature paints the best pictures. A little corner of paradise in my backyard.',
    initialLikes: 967,
    comments: [
      { user: 'botanical_life', text: 'What flower is that?' },
    ],
    commentCount: 24,
  },
  {
    ...
    caption: 'Chasing horizons and collecting moments. The ocean never disappoints.',
    initialLikes: 2104,
    comments: [
    ...
    >
      ...
      >
        <img
          src={post.image}
          alt="Post"
          className="aspect-square w-full object-cover bg-gray-800"
        />
        <motion.div
      ...
      <div className="p-4 space-y-3">
        <div className="flex items-center gap-4">
          ...
          >
            ...
            >
              <Heart
                className={`w-5 h-5 transition-colors duration-200 ${liked ? 'fill-red-500 text-red-500' : 'text-gray-400 hover:text-red-400'}`}
              />
            </motion.div>
        ...
        {post.comments.slice(0, 2).map((comment, ci) => (
          <p key={ci} className="text-sm text-gray-400">
            <span className="font-semibold text-gray-300 mr-2">{comment.user}</span>
            {comment.text}
          </p>
        ))}
      </div>
...
export default function MockFeedSection() {
  return (
    <section className="relative min-h-screen py-32 bg-gradient-to-b from-gray-950 via-blue-950/20 to-gray-950">
      <motion.h2
