    ...
    >
      <div className="relative mb-4 inline-block">
        <img
          src={creator.avatar}
          alt={creator.name}
          className="w-16 h-16 rounded-full object-cover ring-2 ring-white/10 group-hover:ring-blue-500/50 transition-all duration-300"
        />
        <div className="absolute -bottom-1 -right-1 w-5 h-5 bg-green-500 rounded-full border-2 border-gray-950" />
      </div>
      <h3 className="font-semibold text-white group-hover:text-blue-400 transition-colors duration-300">{creator.name}</h3>
      <p className="text-sm text-blue-400 mb-1">{creator.handle}</p>
      <p className="text-gray-400 text-sm mb-2">{creator.role}</p>
      <p className="text-xs text-gray-500 mb-3">{creator.followers} followers</p>
      <p className="text-sm text-gray-400 leading-relaxed">{creator.bio}</p>
      <motion.div
        className="h-0.5 w-0 bg-gradient-to-r from-blue-500 to-cyan-500 rounded-full mt-4 group-hover:w-full transition-all duration-500"
      />
    </motion.div>
  )
}
...
export default function CreatorsSection() {
  return (
    <section className="py-32 overflow-hidden relative">
      <div className="absolute inset-0 bg-gradient-to-b from-gray-950 via-blue-950/10 to-gray-950 pointer-events-none" />
      <div className="relative z-10 mb-16">
        ...
        <motion.p
          className="text-gray-400 text-center text-lg max-w-xl mx-auto"
          initial={{ opacity: 0 }}
